...
namespace FirstAvaloniaApp.ViewModels
{
    public partial class MainWindowViewModel : ViewModelBase
    {
        ...
        private void AddSampleTodos()
        {
            var items = new[]
            {
                new TodoItemViewModel(new TodoItem { Title = "Create UI mockups", Description = "Design the interface for the new dashboard feature" }),
                new TodoItemViewModel(new TodoItem { Title = "Finish project proposal", Description = "Complete the budget section and timeline estimates", IsCompleted = true }),
                new TodoItemViewModel(new TodoItem { Title = "Review pull requests", Description = "Check code changes and provide feedback to the team" }),
                new TodoItemViewModel(new TodoItem { Title = "Attend weekly meeting", Description = "Prepare progress report for team status meeting", CreatedAt = DateTime.Now.AddDays(-1) }),
                new TodoItemViewModel(new TodoItem { Title = "Research new technologies", Description = "Evaluate potential tech stack for the upcoming project" })
            };
